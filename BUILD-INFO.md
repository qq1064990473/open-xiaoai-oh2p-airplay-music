...
- 发布文件：`client-airplay-music-ha-kuwo-ranking-oh2p-armv7-glibc2.25-20260912`。
- 发布二进制大小：3,242,936 bytes。
- SHA-256：发布时见 `dist/SHA256SUMS-v1.4.0-airplay-music-ha.5-kuwo-ranking.txt`。
- 本次构建在 Docker 的 Rust 1.96 + Zig 0.16 环境完成；构建目标仍为 `armv7-unknown-linux-gnueabihf.2.25`。
