...
pub struct MusicSearchConfig {
    /// Preferred catalogue for ordinary song/artist searches: `kuwo` or `qq_music`.
    pub provider: String,
    /// Catalogue used when the preferred search endpoint fails or has no usable results.
...
pub struct MusicPlayUrlConfig {
    /// Kuwo direct-stream sources. They accept a numeric Kuwo RID, not `MUSIC_<rid>`.
    pub kuwo_primary_enabled: bool,
    pub kuwo_primary_url_template: String,
    pub kuwo_backup_enabled: bool,
    pub kuwo_backup_url_template: String,
    pub kuwo_level: String,
    /// Legacy QQ URL resolvers, used only for `qq:` catalogue entries.
...
impl Default for MusicPlayUrlConfig {
    fn default() -> Self {
        ...
        Self {
            kuwo_primary_enabled: true,
            kuwo_primary_url_template:
                "https://musicapi.haitangw.net/music/kw.php?type=mp3&id={id}&level={level}".into(),
            kuwo_backup_enabled: true,
            kuwo_backup_url_template:
                "http://music.nxinxz.com/kw.php?id={id}&level={level}&type=mp3".into(),
            kuwo_level: "exhigh".into(),
